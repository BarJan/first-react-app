...
function ActorPage(){
    ...
    // filtering the cards by the Filter state
    function FilterCards(filter) {
        setActorsData(actorsData.filter(actor => actor.fname.toLowerCase().includes("".toLowerCase())));
    }
    //build all columns of the page, each contains 1 card
    
    const actorsCards = actorsData.map( actor => <Col><Actor actor={actor} /></Col>);
